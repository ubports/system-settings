  ...
  for (int i = 1; i < arguments.count(); i++) {
    ...
    if (argument.startsWith("settings://")) {
      ...
      Q_FOREACH (const StringPair &pair, query.queryItems()) {
        if (!pluginOptions.contains(pair.first)) {
          pluginOptions.insert(pair.first, pair.second);
        } else if (pluginOptions.value(pair.first).typeName() == "QString") {
          QStringList list;
          list << pluginOptions.value(pair.first).toString() << pair.second;
          pluginOptions.insert(pair.first, list);
        } else {
          QStringList list = pluginOptions.value(pair.first).toStringList();
          list << pair.second;
          pluginOptions.insert(pair.first, list);
        }
      }
